...
## The bundled reference module has been removed

Earlier versions shipped a small reference module in the repository (under
`app/Modules/`), disabled by default. It has been **removed**: Core now ships
with no bundled business module at all, and `config/penova.php` `modules` is
empty.

- **Impact.** A fresh install is unaffected - Core was already complete on its
  own and enabled no module by default. Only an application that had explicitly
  wired the old reference module into `config/penova.php` is affected.
- **Recovery path.** Remove the stale provider line from your `config/penova.php`
  `modules` array. Business capabilities now come from modules you add yourself -
  see `app/Modules/README.md` for the module contract.
