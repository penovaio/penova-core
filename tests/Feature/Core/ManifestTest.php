...
test('a manifest declares identity and exposes it', function () {
    $manifest = Manifest::for('blog', 'Blog', 'A blog.', '0.1.0');

    expect($manifest->key())->toBe('blog');
...
test('a manifest requires every identity field', function () {
    expect(fn () => Manifest::for('', 'Blog', 'x', '0.1.0'))->toThrow(InvalidArgumentException::class);
    expect(fn () => Manifest::for('blog', '', 'x', '0.1.0'))->toThrow(InvalidArgumentException::class);
    expect(fn () => Manifest::for('blog', 'Blog', '', '0.1.0'))->toThrow(InvalidArgumentException::class);
    expect(fn () => Manifest::for('blog', 'Blog', 'x', ''))->toThrow(InvalidArgumentException::class);
});

test('a manifest declares menu, widget and permission sections', function () {
    $manifest = Manifest::for('blog', 'Blog', 'x', '0.1.0')
        ->menu([['key' => 'blog']])
        ->widgets([['key' => 'blog-widget']])
        ->permissions(['blog.view']);

    expect($manifest->menuItems())->toBe([['key' => 'blog']]);
    expect($manifest->widgetDescriptors())->toBe([['key' => 'blog-widget']]);
    expect($manifest->permissionSlugs())->toBe(['blog.view']);
});
...
test('a manifest is declaration-like: declaring a section does not mutate the original', function () {
    $base = Manifest::for('blog', 'Blog', 'x', '0.1.0');
    $withMenu = $base->menu([['key' => 'blog']]);

    expect($base->menuItems())->toBe([]);                       // original untouched
    expect($withMenu->menuItems())->toBe([['key' => 'blog']]); // new instance carries it
    expect($withMenu)->not->toBe($base);
});

test('a manifest serialises to its named sections', function () {
    $manifest = Manifest::for('blog', 'Blog', 'x', '0.1.0')->permissions(['blog.view']);
